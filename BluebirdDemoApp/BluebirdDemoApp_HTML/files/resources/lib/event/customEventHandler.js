        ...
        if (elem) {
            ...
            switch (elem.prop("tagName")) {
                ...
                case "TEXTAREA":
                    ...
                    break;
                case "A":
                    if (elem.hasClass('ui-link-inherit') || (elem.parent().parent() && elem.parent().parent().hasClass("ui-listview"))) {					
...
function setVar_(key, id, property, src, context) {
    ...
    if (attr == 'text') {
        value = getText(elem[0].id, src).trim();
    }
    else if (elem.data('role') === 'slider' && property === 'toggled') {
        value = elem.val();
    }
    else if (elem.data('role') === 'fieldcontain' && property === 'selectedOption') {
        value = [];
        elem.find('.ui-controlgroup-controls input').each(function () {
            var $this = $(this),
                checked = $this.closest('.ui-controlgroup-controls > span').getAttr("checked");
            if(checked) {
                value.push($this.val());
            }
        });
        ...
        if (jQuery.inArray(name, __GLOBAL_ATTRIBUTES) > -1 || jQuery.inArray(name, __JQM_DATA_ATTRIBUTES) > -1) {
            ...
            if (elem[0].tagName === "A" && name === "src" && elem.find("img:first")) {
                ...
                var elem = $(document).find('[id="' + id + '"]'),
                    ...
                    videoId = elem.attr("videoid"),
                    startTime = elem.attr("starttime"),
                    endTime = elem.attr("endtime");

                switch (name) {
                    case 'videoId':
                        elem.attr("videoid", val);
                        videoId = val;
                        break;
                    case 'startTime':
                        elem.attr("starttime", val);
                        startTime = val;                    
                        break;
                    case 'endTime':
                        elem.attr("endtime", val);
                        endTime = val;
                        break;
                    default:
                        elem.prop(name, val);
                ...
                if (ytObj.cueVideoById && videoId && videoId != null) {
                    ...
                    if (endTime && endTime != null) {
                        ...
                        ytObj.cueVideoById(videoId, parseInt(startTime), "default");
                    }
                } else {
                    elem.prop(name, val);
                }
