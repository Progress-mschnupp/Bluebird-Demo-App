(function ($) {
    ...
    apperyFunction.__registerComponent = apperyFunction.__registerComponent = function (id, component) {
     ...
     * @class
     */
    Appery = $t = window.Appery = window.Tiggzi = window.$t = window.Progress = window.Tiggr = apperyFunction;
    ...
    Appery.RenderVisitor = $t.createClass($t.MappingVisitor, /** @lends Appery.RenderVisitor.prototype */ {
        ...
        beforeArrayElementVisit: function (arrayIndex) {
            ...
            tmplComponents.each(function (componentIdx, component) {
                ...
                if (dsid != undefined) {
                    ...
                    var clonedComponent;
                    if (component.attr('data-role') === 'table') {
                        var idx = $.inArray(component.attr('id'), sourceTables);
                        if (idx>-1) {
                            var clonedTable = clonedTables[idx];
                            //single-row case
                            //clonedComponent = that.__specialClone(component.children("tbody").children("tr")[0]);
                            //clonedComponent.appendTo(clonedTable.children("tbody")[0]);
                            for (i=0; i<component.children("tbody").children("tr").length; i++) {
                                clonedComponent = that.__specialClone(component.children("tbody").children("tr").get(i));
                                clonedComponent.appendTo(clonedTable.children("tbody")[0]);
                            ...
                            clonedComponent = that.__specialClone(component);
                            clonedComponent.appendTo(component.parent());
                            sourceTables[sourceTables.length] = component.attr('id');
                            clonedTables[clonedTables.length] = clonedComponent;
                        }
                    } else {
                        clonedComponent = that.__specialClone(component);
                        clonedComponent.appendTo(component.parent());
        ...
        __specialClone: function (originalComponent) {
            ...
            if (originalComponent.jquery && originalComponent.attr('data-role') === 'collapsible') {
                ...
                clonedComponent.children('label').text(originalComponent.children('div').children('label').text());
        ...
        beforeArrayVisit: function (data, elementId) {
            ...
            if (components.length > 0) {
				for(var i=0;i<components.length;i++){
					var parentComplexObject = $(components[i]).attr("rerender");	
					if(parentComplexObject != undefined && Appery(parentComplexObject) != undefined 
						&& typeof Appery(parentComplexObject).removeComponent === 'function'){
						 Appery(parentComplexObject).removeComponent($(components[i]));
					}else{
					  $(components[i]).remove();	
					}
    ...
    Appery.DataSource = $t.createClass(null, /** @lends Appery.DataSource.prototype */{
            ...
            __onlineHandler: function () {

                if (this.options.online) {

                    this.options.online.apply(this, $.merge([], arguments));

                }

            },
			
            __offlineHandler: function () {

                if (this.options.offline) {

                    this.options.offline.apply(this, $.merge([], arguments));
            ...
            __buildRequestSettings: function (settings) {
                var handlers = {
                    ...
                    'online': $.proxy(this.__onlineHandler, this),
                    'offline': $.proxy(this.__offlineHandler, this),										
    ...
    Appery.RestService = $t.createClass(null, /** @lends Appery.RestService.prototype */ {
        ...
        paramPattern: /\{([^{"':]+?)\}/g,
        //pattern for getting part of params without changable part in braces{}
        paramConstantPattern:/([^\}\{]{1,})+?(?=\{)|([^\}\{]{1,})$/g,
...
//            dataType: 'json',
        ...
        __performHeaderParametersSubstitutions: function (settings) {
            if (!$.isEmptyObject(settings.headers)) {
                ...
                for (param in settings.headers) {
                    ...
                    if ($.inArray(param, ["progress-key", "progress-rest", "progress-proxy-url"]) > -1) continue;
        ...
        __performURLSubstitutions: function (settings) {
            ...
            if (settings.headers && "progress-proxy-url" in settings.headers) {
                urlBuffer = settings.headers["progress-proxy-url"];
            ...
            if (urlBuffer) {
                ...
                function __performParamEncodeURI(str) {
                ...
                if (settings.headers && "progress-proxy-url" in settings.headers) {
                    getParamsPart = getParamsPart && getParamsPart.replace(this.paramConstantPattern, __performParamEncodeURI);
                }
                isEncoded = true;
                ...
                if (settings.headers && "progress-proxy-url" in settings.headers) {
                    settings.headers["progress-proxy-url"] = urlBuffer;
                } else if (settings.url) {
    ...
    Appery.ContactsService = $t.createClass(null, /** @lends Appery.ContactsService.prototype */ {
        ...
        process: function (settings) {
            if (this.__requestOptions.echo) {
                ...
                showSpinner();
                // "multiple" value type should be bool. (Default: 'false')
                if(typeof(settings.data.options.multiple) === 'string' && settings.data.options.multiple === "true")
                	settings.data.options.multiple = true;
     ...
     * @class
     */
    Progress.ProgressMapComponent = Appery.ApperyMapComponent = $t.TiggziMapComponent = $t.TiggrMapComponent =
    ...
    Progress.ProgressMobileAudioComponent = Appery.ApperyMobileAudioComponent = $t.TiggziMobileAudioComponent = $t.TiggrMobileAudioComponent =
     ...
     * @class
     */
    Progress.ProgressMobileCarouselComponent = Appery.ApperyMobileCarouselComponent = $t.TiggziMobileCarouselComponent = $t.TiggrMobileCarouselComponent =
        $t.createClass($t.ApperyWrapper, /** @lends Appery.ApperyMobileCarouselComponent.prototype */ {
            ...
            initializeCarousel: function () {
                ...
                if (currentPage[0] == $("body").pagecontainer("getActivePage")[0]) {
					...
					if(currentPage.hasClass("progress_popup")){
						currentPage.bind("popupafteropen", initCarouselWidget);
					}else{
                        if (this.carouselRoot.parent().parent().hasClass("ui-panel")){
                            this.carouselRoot.parent().parent().off('panelopen.carousel').on('panelopen.carousel', initCarouselWidget);                            
                        }else{
     ...
     * @class
     */
    Progress.ProgressMobileDatePickerComponent = Appery.ApperyMobileDatePickerComponent = $t.TiggziMobileDatePickerComponent = $t.TiggrMobileDatePickerComponent =
    ...
    Appery.setDetailContent = function (pageUrl) {
        ...
        if ($("#" + Appery.CurrentScreen + " .content-primary").length ||
			$("#" + Appery.CurrentScreen).closest(".content-primary").length) {
        ...
        function processResponce(data) {
			...
			}else if ($("#" + Appery.CurrentScreen).closest(".content-primary").length){
				selector = "#" + $("#" + Appery.CurrentScreen).closest(".content-primary").closest(".ui-page").attr("id") + " .content-primary";
			}								
		...
		$(selector).closest("div[data-role='page']").children(":jqmData(role='header'):first").find(".ui-title").text(pageTitle);
            }
		...
		if($(selector).closest("div[data-role='page']").hasClass("ui-page-active")){				
...
//      ctx.find("input[data-type='range']").slider("refresh");
    ...
    Appery.openPopup = function (popupScreenId,originEl){
        var popUpScreenSelector = "#"+popupScreenId;
        var popup = $(popUpScreenSelector);
        var parentScreen = Appery.CurrentScreen;
        if (parentScreen == popupScreenId){
        	console.log("parentScreen equal popupScreen");
        	return;
        }
        if($("#"+parentScreen).hasClass("detail-content")) {
            //Look for master-detail page
            var topPage = $("#"+parentScreen).parent().closest("[data-role=page]");
            if(topPage.length>0) {
                parentScreen = topPage.attr("dsid");
            }
        }

        function bindClosePopupHandler(popupSelector, callback) {
        	$(popupSelector).unbind("popupafterclose.inner").bind("popupafterclose.inner",function(){
                var parentScreenSelector = "#"+$(popupSelector).attr("parent");
                $(parentScreenSelector).trigger("pagebeforeshow").trigger("pageshow");
                $(popupSelector).removeAttr("parent");
        ...
        function getPositionOptions(popupEL,el){
			if(el != undefined && popupEL.attr("data-position-to") == "origin"){
				var offset = $(el).offset();
				if(offset != undefined){
					var options = {};
					options.x = offset.left + $(el).outerWidth() / 2;
					options.y = offset.top + $(el).outerHeight() / 2;				
					return options;
				}
			}
			return undefined;
        ...
        function positioningPopup(){
            var popupEL = $(popUpScreenSelector);
            var popupPosition = popupEL.attr("data-position-to");
            popupEL.popup("reposition", {positionTo: popupPosition});
        }

        //fixed issue with positioning popups after orientation change
        $.mobile.window.off("throttledresize",positioningPopup).on("throttledresize",positioningPopup);

		if(!Appery.isAppRenderedAsOnePage){
			//app is rendered as separate pages
			//load popup screen from external document and show it
			if($(popUpScreenSelector).size() == 0){									
				$.mobile.loadPage(popupScreenId + ".html",{role:"page",pageContainer :$(":mobile-pagecontainer")}).done(function (e, ui, page) {						
					$(page).attr("data-role","popup");
					page.removeClass("ui-page");					
					$(page).popup()
					$(page).attr("parent",parentScreen);
					bindClosePopupHandler(popUpScreenSelector);
					var options = getPositionOptions($(page),originEl);
					...
					console.error("There is an error occurs during popup screen load.");
				});	
				return;				
			}
		}else{
		    
            var oldParent = popup.parent(),
                oldDataRole = popup.attr("data-role");
		    
			//app is rendered as one page
			//in this case we need to initialize popup screen content 
			//manually via page() widget and show it
			if(!popup.hasClass("jqminitialized")){
			    popup.page();
                $("#"+parentScreen).append(popup);
                popup.attr("data-role","popup");
                popup.removeClass("ui-page");
                popup.popup();
                popup.addClass("jqminitialized");
			}else{
				if(popup.closest("[data-role='page']").attr("id") != parentScreen){
				    popup.attr("data-role","popup");
				    popup.popup("destroy");
					$("#"+parentScreen).append(popup);
					popup.popup();
            ...
            var closeCallback = function () {
                oldParent.append(popup);
                popup.attr("data-role", oldDataRole);
                popup.removeClass("ui-popup");
            };

            bindClosePopupHandler(popUpScreenSelector, closeCallback);
			
		}		
		
		$(popUpScreenSelector).attr("parent",parentScreen);
		var options = getPositionOptions($(popUpScreenSelector),originEl);
		if(options != undefined){
			$(popUpScreenSelector).popup("open",options);						
		}else{
			$(popUpScreenSelector).popup("open");												
		}
        

    }
    
    Appery.closePopup = function (){
            ...
            if (el.attr("tiggzitype") != undefined && el.attr("tiggzitype") == "marker") {
                ...
                if (value == "true" || value == "visible") {
                    if (el.css('display') != "none") {
                        ...
                        } else if (el.attr("data-role") == "table") {
                            display = "table";
                        }
    ...
    $.fn.setText = function (str) {
        if (this.length > 0) {
			if (this.children('.ui-radio').length)
                this.find('.ui-radio label').text(str)
            else if (this.children('.ui-checkbox').length)
                this.find('.ui-checkbox label').text(str)
